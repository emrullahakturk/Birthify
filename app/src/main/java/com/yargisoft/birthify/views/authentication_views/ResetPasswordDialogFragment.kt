...
import com.yargisoft.birthify.UserFrequentlyUsedFunctions.isValidEmail
...
import com.yargisoft.birthify.databinding.FragmentResetPasswordDialogBinding
...
class ResetPasswordDialogFragment : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        binding = DataBindingUtil.inflate(layoutInflater, R.layout.fragment_reset_password_dialog, null, false)
        ...
        val resetPasswordEditText:  EditText = binding.resetPasswordEditText
        ...
        binding.resetPasswordButton.setOnClickListener {
            val resetEmail = resetPasswordEditText.text.toString()
            ...
            disableViewEnableLottie(lottieAnimationView, dialog.findViewById(android.R.id.content))
                ...
                if(isValidEmail(resetEmail)){
                    ...
                    Handler(Looper.getMainLooper()).postDelayed({
                        this@ResetPasswordDialogFragment.lifecycleScope.launch {
                            var isLoadedEmitted = false // Kontrol değişkeni

                            this@ResetPasswordDialogFragment.lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
                                authViewModel.isLoaded.collect { isLoaded ->
                                    if (isLoaded && !isLoadedEmitted) {
                                        ...
                                        if (isSuccess) {
                                            Snackbar.make(dialog.findViewById(android.R.id.content), "Reset e-mail succesfully sent", Snackbar.LENGTH_SHORT).show()
                                            delay(2000)
                                            dialog.dismiss()
                                        } else {
                    ...
                    Snackbar.make(dialog.findViewById(android.R.id.content), "Please enter a valid e-mail.", 2000).show()
                    enableViewDisableLottie(lottieAnimationView, dialog.findViewById(android.R.id.content))
                }
            }
