...
import com.yargisoft.birthify.data.sharedpreferences.UserConstants.IS_LOGGED_IN_KEY
import com.yargisoft.birthify.data.sharedpreferences.UserConstants.NAME_KEY
...
class AuthRepository @Inject constructor(
    ...
    private fun updateToken(userId: String, token: String, callback: (Boolean) -> Unit) {
        firestore.collection("users").document(userId).update("token", token)
            .addOnCompleteListener { task ->
                                ...
                                if (firestoreTask.isSuccessful) {
                                    ...
                                    onFailure(
                                        firestoreTask.exception?.message
                                            ?: "Password update in Firestore failed."
                                    )
                                }
                            }
                    } else {
                        onFailure(
                            passwordUpdateTask.exception?.message ?: "Password update failed."
                        )
                    }
                }
            } else {
                    ...
                    if (deleteTask.isSuccessful) {
                        ...
                        user.delete().addOnCompleteListener { authTask ->
                            if (authTask.isSuccessful) {
                                ...
                                onFailure(
                                    authTask.exception?.message ?: "User deletion failed."
                                )
                            }
                        }
                    } else {
            ...
            firestore.collection("users").document(userId).get().addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    if (document != null && document.exists()) {
                        ...
                        val recordedDate = document.getTimestamp("recordedDate") ?: Timestamp(
